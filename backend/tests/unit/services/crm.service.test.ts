import { getProductsFromCRM, getProductFromCRM, createOrderInCRM, checkCRMHealth } from '../../../src/services/crm.service';
...
describe('CRM Service', () => {
  ...
  describe('getProductsFromCRM', () => {
    it('should return products from cache if available', async () => {
      ...
      const result = await getProductsFromCRM({ category: 'test' });
  ...
  describe('getProductFromCRM', () => {
    it('should return product from cache if available', async () => {
      ...
      const result = await getProductFromCRM(1);
